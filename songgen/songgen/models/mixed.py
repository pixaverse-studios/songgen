...
# @dataclass
# class SongGenUnconditionalInput(ModelOutput):
#     """
#     Args:
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenSinusoidalPositionalEmbedding with Musicgen->SongGen
class SongGenSinusoidalPositionalEmbedding(nn.Module):
...
# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->SongGen
class SongGenRotaryEmbedding(nn.Module):
...
class SongGenAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[SongGenDecoderConfig] = None,
    ):
        super().__init__()
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenFlashAttention2 with Musicgen->SongGen
class SongGenFlashAttention2(SongGenAttention):
    """
    SongGen flash attention module. This module inherits from `SongGenAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # SonGenFlashAttention2 attention does not support output_attentions
...
# Copied from transformers.models.bart.modeling_bart.BartSdpaAttention with Bart->SongGen
class SongGenSdpaAttention(SongGenAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "SongGenModel is using SongGenSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
SONGGEN_ATTENTION_CLASSES = {
    "eager": SongGenAttention,
    "sdpa": SongGenSdpaAttention,
    "flash_attention_2": SongGenFlashAttention2,
}


class SongGenDecoderLayer(nn.Module):
    def __init__(self, config: SongGenDecoderConfig, layer_idx: int = None):
        super().__init__()
        self.embed_dim = config.hidden_size

        self.self_attn = SONGGEN_ATTENTION_CLASSES[config._attn_implementation](
        ...
        self.encoder_attn = SONGGEN_ATTENTION_CLASSES[cross_attn_implementation](
            self.embed_dim,
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenPreTrainedModel with Musicgen->SongGen
class SongGenPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = SongGenDecoderConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _supports_flash_attn_2 = True
    _supports_sdpa = True
    _no_split_modules = ["SongGenDecoderLayer", "SongGenAttention"]
...
MUSICGEN_START_DOCSTRING = r"""
    Parameters:
        config ([`SonGenConfig`]): Model configuration class with all the parameters of the model.
...
class SongGenDecoder(SongGenPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`SongGenDecoderLayer`]
    """

    def __init__(self, config: SongGenDecoderConfig):
        ...
        if self.track_pattern.startswith('parallel'):
            self.num_embed_tokens = config.num_codebooks * 2
        else:
            self.num_embed_tokens = config.num_codebooks
        logger.info(f'****SongGenDecoder track_pattern:{self.track_pattern}, num_embed_tokens:{self.num_embed_tokens}, num_codebooks: {self.num_codebooks}')
        self.embed_tokens = nn.ModuleList(
            [nn.Embedding(embed_dim, config.hidden_size) for _ in range(self.num_embed_tokens)]
        )

        self.rope_embeddings = config.rope_embeddings
        if not config.rope_embeddings:
            self.embed_positions = SongGenSinusoidalPositionalEmbedding(
                config.max_position_embeddings,
                config.hidden_size,
            )
        else:
            self.rotary_emb = SongGenRotaryEmbedding(
                config.hidden_size // config.num_attention_heads,
                max_position_embeddings=config.max_position_embeddings,
                base=config.rope_theta,
            )
        self.layers = nn.ModuleList(
            [SongGenDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            # (bsz * num_embed_tokens, seq_len) -> (bsz, num_embed_tokens, seq_len)
            input = input_ids.reshape(-1, self.num_embed_tokens, input_ids.shape[-1])
        ...
        if inputs_embeds is None:
            inputs_embeds = sum([self.embed_tokens[codebook](input[:, codebook]) for codebook in range(self.num_embed_tokens)]) / int(self.num_embed_tokens / self.num_codebooks)
        
            
        #Construct prefix, fuse with inputs_embeds, and update input shape.
        prepended_sequence_length = 0
        prefix_attention_mask = None
        prefix_hidden_states = None
      

        if prompt_hidden_states is not None: #Prepend lyrics before the audio tokens.
            prefix_hidden_states = prompt_hidden_states
            prefix_attention_mask = prompt_attention_mask
            prepended_sequence_length += prefix_hidden_states.shape[-2]
            inputs_embeds = torch.cat([prefix_hidden_states, inputs_embeds], dim=1)
        ...
        # i.i.f `prompt_cross_attention=True`. SongGenForConditionalGeneration's taking care of setting `prompt_attention_mask=None`
        if prefix_attention_mask is not None and attention_mask is not None:
            attention_mask = torch.cat([prefix_attention_mask, attention_mask], dim=1)
        elif prefix_attention_mask is not None:
            logger.warning_once(
                "`prefix_attention_mask` is specified but `attention_mask` is not. A full `attention_mask` will be created. Make sure this is the intended behaviour."
            )
            if past_key_values_length == 0:
                attention_mask = torch.cat(
                    [
                        prefix_attention_mask,
                        torch.ones(input_shape, device=self.device, dtype=prefix_attention_mask.dtype),
                    ],
                    dim=1,
                )
            else:
                # In the generation case of `prompt_cross_attention=False`, we need to recreate an attention mask from scratch
                ...
                generated_length = past_key_values_length - prefix_attention_mask.shape[1] + 1
                attention_mask = torch.cat(
                    [
                        prefix_attention_mask,
                        torch.ones(
                            (input_shape[0], generated_length), device=self.device, dtype=prefix_attention_mask.dtype
                        ),
                    ],
                    dim=1,
                )
...
@add_start_docstrings(
    "The bare SongGen decoder model outputting raw hidden-states without any specific head on top.",
    MUSICGEN_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenModel with Musicgen->SongGen
class SongGenModel(SongGenPreTrainedModel):
    def __init__(self, config: SongGenDecoderConfig):
        super().__init__(config)
        self.decoder = SongGenDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
...
@add_start_docstrings(
    "The SongGen decoder model with a language modelling head on top.",
    MUSICGEN_START_DOCSTRING,
)
class SongGenForCausalLM(SongGenPreTrainedModel):
    def __init__(self, config: SongGenDecoderConfig):
        super().__init__(config)

        self.model = SongGenModel(config)
        ...
        logger.info(f'****SongGenForCausalLM  track_pattern:{self.track_pattern}, num_heads:{self.num_heads}, num_codebooks: {self.num_codebooks}')

        self.lm_heads = nn.ModuleList(
            [nn.Linear(config.hidden_size, config.vocab_size, bias=False) for _ in range(self.num_heads)]
        )
        
        self.add_vocal_loss =  getattr(config, 'add_vocal_loss', False)
        logger.info(f'****SongGenForCausalLM  add_vocal_loss:{self.add_vocal_loss}')

        if self.add_vocal_loss:
            assert self.track_pattern == 'mixed_pro', f"add_vocal_loss = True, but track_pattern = {self.track_pattern}, != mixed_pro"
            self.vocal_lm_heads = nn.ModuleList(
    ...
    def forward(
        ...
        vocal_labels: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        hidden_states = outputs[0] #(bsz, seq_len, 1024)

        lm_logits = torch.stack([head(hidden_states) for head in self.lm_heads], dim=1) #(bsz, num_codebook, seq_len, 1088)
        ...
        vocal_loss = None
        vocal_codebook_losses = None 
        if self.add_vocal_loss and vocal_labels is not None: 
            assert vocal_labels.shape == labels.shape, f'vocal_labels: {vocal_labels.shape}, labels: {labels.shape}'
            vocal_logits = torch.stack([head(hidden_states) for head in self.vocal_lm_heads], dim=1)[:, :, -vocal_labels.shape[1] :]

            vocal_loss_fct = CrossEntropyLoss()
            vocal_loss = torch.zeros([], device=self.device)
            vocal_codebook_losses = []

            # (bsz, vocab_size, seq_len, num_codebooks), (bsz, seq_len, num_codebooks)
            vocal_labels = vocal_labels.masked_fill(vocal_labels == self.config.bos_token_id, -100)

            # we use every codebooks token AND one single EOS at the end of each codebooks
            vocal_mask = (input_ids.transpose(1, 2) != self.config.eos_token_id) & ((vocal_labels != -100))

            # per codebook cross-entropy
            for codebook in range(self.config.num_codebooks):
                vocal_codebook_logits = vocal_logits[:, codebook].contiguous().view(-1, vocal_logits.shape[-1])
                vocal_codebook_mask = vocal_mask[..., codebook].contiguous().view(-1)
                vocal_codebook_labels = vocal_labels[..., codebook].contiguous().view(-1)

                vocal_codebook_loss = vocal_loss_fct(vocal_codebook_logits[vocal_codebook_mask], vocal_codebook_labels[vocal_codebook_mask])
                vocal_codebook_losses.append(vocal_codebook_loss)
                vocal_loss += vocal_codebook_loss

            vocal_loss = vocal_loss / self.config.num_codebooks
        ...
        return CausalLMOutputWithCrossAttentions(
            ...
            vocal_codebook_losses=vocal_codebook_losses,
        )
    
    #TODO The original version of parler-tts has not yet been modified
...
@add_start_docstrings(
    "The composite SongGen model with a text encoder, audio encoder and SongGen decoder, "
...
class SongGenMixedForConditionalGeneration(PreTrainedModel):
    ...
    # Simple, in the init function, we are basically doing three things:
    # 1. We verify that the required configuration is provided, and we initialize the Decoder, TextEncoder, and AudioEncoder.
    # 2. We make sure that the individual model's config refers to the shared config, so that the updates to the config will be synced.
    # 3. We initialize the projection and embedding layers.
    # 4. We initialize the lyrics encoder and the prompt cross attention.
    # 5. We initialize the MERT model and the audio MERT to decoder projection.
    # 6. We do a sanity check to make sure that the text encoder does not have a language model head, so that it does not do token prediction
    # 7. We call the post_init function to do some more configuration
    def __init__(
        self,
        config: Optional[SongGenConfig] = None,
        text_encoder: Optional[PreTrainedModel] = None,
        audio_encoder: Optional[PreTrainedModel] = None,
        decoder: Optional[SongGenForCausalLM] = None, 
        mert_model: Optional[PreTrainedModel] = None,  
    ):
        if config is None and (text_encoder is None or audio_encoder is None or decoder is None):
            raise ValueError(
                "Either a configuration has to be provided, or all three of text encoder, audio encoder and SongGen decoder."
            )
        if config is None:
            config = SongGenConfig.from_sub_models_config(text_encoder.config, decoder.config) 
        else:
        ...
        if config.decoder.cross_attention_hidden_size is not None:
            if config.decoder.cross_attention_hidden_size != config.text_encoder.hidden_size:
                raise ValueError(
                    "If `cross_attention_hidden_size` is specified in the SongGen decoder's configuration, it has to be equal"
        ...
        if decoder is None:
            decoder = SongGenForCausalLM(config.decoder)
        ...
        self.lyrics_encoder = ConformerEncoder(input_size=self.decoder.config.hidden_size, static_chunk_size=0 ) 
        self.lyrics_to_dec_proj = nn.Linear( self.lyrics_encoder.output_size, self.decoder.config.hidden_size)
        ...
        # This basically calls _init_weights and tie_weights, and does some more configuration
        self.post_init()
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        ```python
        >>> from songgen import SongGenForConditionalGeneration

        >>> model = SongGenForConditionalGeneration.from_pretrained("...")
        ```"""

        # At the moment fast initialization is not supported for composite models
        if kwargs.get("_fast_init", False):
            logger.warning(
                "Fast initialization is currently not supported for SongGenForConditionalGeneration. "
    ...
    def from_sub_models_pretrained(
        ...
        **kwargs,
        ...
        r"""
        Instantiate a text encoder, an audio encoder, and a SongGen decoder from one, two or three base classes of the
        ...
        if decoder is None:
            ...
            if "config" not in kwargs_decoder:
                decoder_config, kwargs_decoder = SongGenDecoderConfig.from_pretrained(
                    decoder_pretrained_model_name_or_path, **kwargs_decoder, return_unused_kwargs=True
                )

                if isinstance(decoder_config, SongGenConfig):
            ...
            decoder = SongGenForCausalLM.from_pretrained(decoder_pretrained_model_name_or_path, **kwargs_decoder)

        # instantiate config with corresponding kwargs
        config = SongGenConfig.from_sub_models_config(
            text_encoder.config, decoder.config, **kwargs #, audio_encoder.config
        )

        mert_path = 'm-a-p/MERT-v1-330M'
        mert_model = AutoModel.from_pretrained(mert_path, trust_remote_code=True)

        return cls(text_encoder=text_encoder, audio_encoder=audio_encoder, decoder=decoder, mert_model=mert_model, config=config)
    
    def encode_lyrics(
        self,
        prompt_input_ids: Optional[torch.FloatTensor] = None,
        prompt_attention_mask: Optional[torch.LongTensor] = None,
        add_prenet: Optional[bool] = True, # False: w/o lyrics_encoder
    ):
        prompt_embs = self.embed_prompts(prompt_input_ids)
        if not add_prenet:
            return prompt_embs
        prompt_prenet_out, _mask = self.lyrics_encoder(prompt_embs, prompt_attention_mask, decoding_chunk_size=1, num_decoding_left_chunks=-1)
        prompt_hidden_states = self.lyrics_to_dec_proj(prompt_prenet_out)
    ...
    def forward(
        ...
        prompt_attention_mask: Optional[torch.LongTensor] = None,
        prompt_hidden_states: Optional[torch.FloatTensor] = None,
        ref_voice_values: Optional[torch.FloatTensor] = None, 
        ref_voice_hidden_states: Optional[torch.FloatTensor] = None, 
        ref_voice_attention_mask: Optional[torch.LongTensor] = None,
        ...
        **kwargs,
        ...
        if prompt_hidden_states is None:
            if prompt_input_ids is not None:
                assert prompt_attention_mask is not None, 'prompt_attention_mask should not be None'
                prompt_hidden_states = self.encode_lyrics(prompt_input_ids, prompt_attention_mask)
        ...
        if encoder_outputs is None:
            if input_ids is not None:
                ...
                # optionally project encoder_hidden_states #Note
                # if (
                #     self.text_encoder.config.hidden_size != self.decoder.config.hidden_size
                #     and self.decoder.config.cross_attention_hidden_size is None
                # ):
            ...
            if prompt_hidden_states is not None and self.prompt_cross_attention:
                if encoder_hidden_states is not None: 
                    ...
                    # concatenate text description states with lyrics states
                    ...
                    encoder_outputs = BaseModelOutput(last_hidden_state=encoder_hidden_states, hidden_states = None, attentions =  prompt_attention_mask )
        ...
        if ref_voice_hidden_states is None and ref_voice_values is not None:  
            self.mert_model.eval()
            with torch.no_grad():
                mert_outputs = self.mert_model(input_values=ref_voice_values, attention_mask=ref_voice_attention_mask, output_hidden_states=True) #[bsz, mert_num_layer, time_steps, 1024 feature_dim]
                ref_voice_features = torch.stack(mert_outputs.hidden_states, dim=1)
        
            bsz, mert_n_layers, mert_t, mert_f = ref_voice_features.shape  #[bsz, mert_num_layer, time_steps, 1024 feature_dim]
            ref_voice_features = ref_voice_features.transpose(1, 2).reshape(-1, mert_n_layers, mert_f)
            ref_voice_hidden_states = self.mert_layer_agg(ref_voice_features).squeeze().reshape(bsz, mert_t, mert_f)  
            ref_voice_hidden_states = self.audio_mert_to_dec_proj(ref_voice_hidden_states) #[bsz, mert_t, 1024]
        
        if ref_voice_hidden_states is not None:
            ref_voice_attention_mask = torch.ones( 
                    ref_voice_hidden_states.shape[:2], device=self.device, dtype=attention_mask.dtype
                )
            if ref_voice_values is not None:
                zero_samples = (ref_voice_values.abs().sum(dim=-1) == 0) 
                ref_voice_attention_mask[zero_samples] = 0
            if attention_mask is None:
                attention_mask = torch.ones(
                    encoder_hidden_states.shape[:2], device=self.device, dtype=ref_voice_attention_mask.dtype
                )
                logger.warning_once(
                    "`ref_voice_attention_mask` is specified but `encoder_attention_mask` is not. A full `encoder_attention_mask` will be created. Make sure this is the intended behaviour."
                )
           
            # concatenate text description states with ref_voice hidden states
            encoder_hidden_states = torch.cat([ref_voice_hidden_states, encoder_hidden_states], dim=1)
            if ref_voice_attention_mask is not None:
                attention_mask = torch.cat([ref_voice_attention_mask, attention_mask], dim=1)
            encoder_outputs["last_hidden_state"] = encoder_hidden_states 


        # labels are given, but decoder_input_ids and decoder_inputs_embeds are not given
        # when in training, we prepare the decoder_input_ids using the labels
        ...
        if (labels is not None) and (decoder_input_ids is None and decoder_inputs_embeds is None):
            ...
            #  - [S, B, c, d, E, E]
            #  - [S, B, B, e, f, E]
            #  - [S, B, B, B, g, h]  in fact  B=S
            decoder_input_ids = shift_tokens_right(
                labels, self.config.pad_token_id, self.config.decoder_start_token_id
            ).transpose(1, 2)

        # no teacher forcing, so we need to generate the decoder_input_ids
        ...
        return Seq2SeqLMOutput(
            ...
            vocal_loss=decoder_outputs.vocal_loss,
            ...
            vocal_codebook_losses=decoder_outputs.vocal_codebook_losses
        )
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, EncoderDecoderCache): 
                ...
                if past_key_values.get_seq_length() > 0: #generate 1st step  past_key_values.get_seq_length()=0
        ...
        if decoder_attention_mask is None and prompt_attention_mask is not None:
            ...
            if past_key_values is None or (
                ...
                # to be able to prepend the prompt attention mask.
                # Since we generate token per token, we can recompute the generated length from the information we have.
                generated_length = past_key_values_length+ 1
                if prompt_attention_mask is not None:
                     generated_length -= prompt_attention_mask.shape[1] 
                decoder_attention_mask = torch.ones(
                    (input_shape[0], generated_length), device=self.device, dtype=torch.long
                )
        ...
        if kwargs is not None and "ref_voice_values" in kwargs:
            ref_voice_values = kwargs.pop("ref_voice_values")
            res["ref_voice_values"] = ref_voice_values
        
        if kwargs is not None and "ref_voice_attention_mask" in kwargs:
            ref_voice_attention_mask = kwargs.pop("ref_voice_attention_mask")
            res["ref_voice_attention_mask"] = ref_voice_attention_mask
    ...
    def freeze_cross(self, freeze_cross = True):
        if freeze_cross:
            ...
            if hasattr(self, 'prompt_cross_attention') and self.prompt_cross_attention: 
                ...
                if hasattr(self, 'lyrics_encoder'):
                    logger.info("- lyrics_encoder")
                    for param in self.lyrics_encoder.parameters():
                        param.requires_grad = False
                    self.lyrics_encoder._requires_grad = False 
    ...
    def generate(
        ...
        **kwargs,
        ...
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList([SongGenLogitsProcessor(generation_config.eos_token_id, self.decoder.num_codebooks, batch_size, inputs_tensor.device)])
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        ...
        #output_ids.shape [bsz*num_codebooks, seq_len]
        # Apply the pattern mask to the final ids
